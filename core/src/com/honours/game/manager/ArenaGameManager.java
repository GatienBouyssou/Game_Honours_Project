...
import com.honours.AI.unsofisticatedAI.DoNothingAI;
import com.honours.AI.unsofisticatedAI.SimpleAI;
...
public class ArenaGameManager implements InputProcessor {
	...
	public ArenaGameManager(ArenaGameScreen screen, boolean isTutorial) {
		...
		teamHuman.addNewPlayer(worldCreator.getRandomSpawnT1(), textureAtlas, "Human", game.getSpellHumans(), PlayerType.Human, MAIN_PLAYER_ID);
		teamAI.addNewPlayer(worldCreator.getRandomSpawnT2(), textureAtlas, "AI", game.getSpellAI(), PlayerType.AI, 1);
		...
		if (game.userHaveInternet()) {			
			aiManager = new CBRManager(world, teams);
		} else if (isTutorial) {
			aiManager = new DoNothingAI(world, teams);
		} else {
			aiManager = new SimpleAI(world, teams);
		}
	...
	public void update(float deltaTime) {
		...
		if (!gameOver) {
			arenaInf.update(teams.get(TEAM_HUMAN).getPlayerById(MAIN_PLAYER_ID), gameTime); 
		}
	}
	...
	public boolean keyDown(int keycode) {
		...
		if (keycode == keyForSpells.get(AUTO_ATTACK_INDEX)) {
			...
			teams.get(TEAM_HUMAN).playerCastSpell(MAIN_PLAYER_ID,AUTO_ATTACK_INDEX, new Vector2(destInWorld.x, destInWorld.y));
		} else if(keycode == keyForSpells.get(SPELL_1_INDEX)) {
			destInWorld = camera.unproject(new Vector3(Gdx.input.getX(), Gdx.input.getY(), 0));
			teams.get(TEAM_HUMAN).playerCastSpell(MAIN_PLAYER_ID,SPELL_1_INDEX, new Vector2(destInWorld.x, destInWorld.y));
		} else if(keycode == keyForSpells.get(SPELL_2_INDEX)) {
			destInWorld = camera.unproject(new Vector3(Gdx.input.getX(), Gdx.input.getY(), 0));
			teams.get(TEAM_HUMAN).playerCastSpell(MAIN_PLAYER_ID,SPELL_2_INDEX, new Vector2(destInWorld.x, destInWorld.y));
		} else if(keycode == keyForSpells.get(SPELL_3_INDEX)) {
			destInWorld = camera.unproject(new Vector3(Gdx.input.getX(), Gdx.input.getY(), 0));
			teams.get(TEAM_HUMAN).playerCastSpell(MAIN_PLAYER_ID,SPELL_3_INDEX, new Vector2(destInWorld.x, destInWorld.y));
		} else if(keycode == keyForSpells.get(SPELL_4_INDEX)) {
			destInWorld = camera.unproject(new Vector3(Gdx.input.getX(), Gdx.input.getY(), 0));
			teams.get(TEAM_HUMAN).playerCastSpell(MAIN_PLAYER_ID,SPELL_4_INDEX, new Vector2(destInWorld.x, destInWorld.y));
		} 
		return true;
	}
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		if (button == Input.Buttons.RIGHT) {
			...
			teams.get(TEAM_HUMAN).playerMoveTo(MAIN_PLAYER_ID, destInWorld.x, destInWorld.y);
		}
		return false;
	}
	...
	public static void playerIsDead(int teamId, int playerId) {
		...
		aiManager.playerDies(team.getPlayerById(playerId));
